...
async function getCourse(slug: string): Promise<Course | null> {
  ...
  const { fetchCourseBySlug } = await import('../../../../lib/supabase');
...
export default async function CourseDetailPage({ params }: { params: { slug: string } }) {
  ...
  // Initialize with empty arrays during build time
  let modules: Module[] = [];
  ...
  if (process.env.VERCEL_ENV !== 'production') {
    ...
    modules = [
      ...
      },
      {
        ...
        description: 'Understand core security concepts.',
        course_id: course.id,
        order_index: 2,
        created_at: new Date().toISOString()
      }
    ...
    const { fetchModulesForCourse, fetchInstructorById } = await import('../../../../lib/supabase');
    
    // Fetch course modules
    modules = await fetchModulesForCourse(course.id) as Module[];
    
    // Fetch instructor info
    instructor = await fetchInstructorById(course.instructor_id) as Instructor | null;
  }
