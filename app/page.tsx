    ...
    <>
      ...
      <section className="relative pt-28 pb-32 overflow-hidden" aria-labelledby="hero-heading">
        <div className="container mx-auto px-4">
          ...
          {/* --- Conditional Loader and Content --- */} 
          {isLoading && (
             <div className="flex justify-center pt-8"> {/* Add some padding */} 
                <TerminalLoader 
                   resourceType="Platform Modules"
                   accentColor="blue" // Use 'blue' which corresponds to neon-blue
                   onComplete={() => setIsLoading(false)}
                />
             </div>
          )}
          {/* --- End Conditional Loader --- */}
        </div>
      </section>
      
      {/* --- Conditionally Render ALL Subsequent Sections --- */}
      {!isLoading && (
        <> {/* Wrap all content sections in a single Fragment */} 
           {/* Key Features Section */} 
           <div className="container mx-auto px-4 -mt-16 mb-16 relative z-10"> {/* Adjust margins for positioning */} 
           ...
           {/* Quick Links Section - Restyled for Dark Theme */}
           <section className="py-20 bg-gray-950 border-t border-gray-800" aria-labelledby="quick-links-heading">
             <div className="container mx-auto px-4">
               <div className="text-center mb-12">
                 <h2 id="quick-links-heading" className="text-3xl md:text-4xl font-bold mb-4 text-white"> {/* Changed text color */}
                   Quick Access
                 </h2>
                 <p className="text-lg text-gray-400 max-w-2xl mx-auto"> {/* Changed text color */}
                   Jump directly to specific resources and community hubs.
                 </p>
               </div>
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto"> {/* Increased max-width */}
                 {/* Define Quick Links Array (can be moved outside component) */}
                 {[ 
                   { href: "/academy/youtube", icon: Youtube, title: "YouTube Channels", description: "Curated video resources.", color: "red" },
                   { href: "/academy/labs", icon: Terminal, title: "Hands-on Labs", description: "Practice simulated skills.", color: "blue" },
                   { href: "/academy/docs", icon: BookOpen, title: "Documentation", description: "Key framework & tool refs.", color: "indigo" },
                   { href: "/community/discord", icon: MessageSquare, title: "Discord Community", description: "Join real-time chat.", color: "cyan" },
                   { href: "/insights/threat-intel", icon: AlertTriangle, title: "Threat Intelligence", description: "Latest threats & vectors.", color: "orange" }, // Corrected path based on previous edits
                   { href: "/insights/news", icon: Newspaper, title: "Cybersecurity News", description: "Stay updated on news.", color: "purple" },
                 ].map((link) => (
                   <Link href={link.href} key={link.href} className="group block">
                      {/* Dark Theme Button/Card Style */}
                      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 h-full transition-all duration-300 hover:border-neon-blue/40 hover:bg-gray-800/60">
                         <div className="flex items-center">
                           <div className={`p-1.5 rounded-md bg-neon-blue/10 text-neon-blue mr-3`}>
                              <link.icon className="w-5 h-5" />
                           </div>
                           <div>
                             <span className="font-semibold text-white group-hover:text-neon-blue transition-colors">{link.title}</span>
