...
export default function ToolsPage() {
  ...
  // Filter tools based on selected category - with improved case insensitivity and safety checks
  const filteredTools = selectedCategory === 'All'
    ? uniqueTools
    : uniqueTools.filter(tool => {
        // Safety check for tool.category
        if (!tool.category) {
          return false;
        }
        
        // Case-insensitive comparison
        const toolCategoryLower = typeof tool.category === 'string' ? tool.category.toLowerCase() : '';
        const selectedCategoryLower = selectedCategory.toLowerCase();
        
        return toolCategoryLower === selectedCategoryLower;
      });
